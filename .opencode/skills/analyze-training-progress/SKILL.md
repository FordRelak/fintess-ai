...
- Для `scope: exercise` указывайте упражнение только в `observation.exerciseId`; `evidence.exerciseId` в таких observations MUST отсутствовать.
- `evidence.exerciseId` разрешён только в `scope: program`, когда evidence нужно отнести к конкретному упражнению внутри общепрограммного observation.
- Если несколько упражнений одновременно подтверждают plateau, сохраняйте отдельное exercise-level observation для каждого упражнения и добавляйте program-level observation как дополнительный общий вывод. Program-level evidence MUST содержать отдельный элемент с `exerciseId` для каждого упражнения, на котором основан общий вывод.
...
8. Перед записью проверьте каждый observation: `scope: exercise` требует `observation.exerciseId` и запрещает `evidence.exerciseId`; `scope: program` запрещает `observation.exerciseId` и допускает `evidence.exerciseId` только для атрибуции упражнения.
9. Если создано program-level plateau, проверьте, что оно дополняет, а не заменяет exercise-level plateau каждого упражнения и содержит evidence для каждого такого упражнения.
10. Запишите `result.json` рядом с локальным входным файлом. Если входной файл недоступен для записи или не является локальным, создайте доступный для скачивания артефакт `result.json`.
