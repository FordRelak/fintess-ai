...
Repository SHALL хранить `eval/result-authoring-guide.md` как человекочитаемую инструкцию для автора `result.json`. Guide SHALL описывать roles `observations`, `hypotheses`, `recommendations` и `limitations`, связи между ними, семантику всех допустимых enum-значений, включая `performance_decline`, ключевые структурные ограничения и проверочный список для result contract `1.1`.
